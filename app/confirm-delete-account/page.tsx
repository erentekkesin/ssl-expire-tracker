...
function ConfirmDeleteAccountContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/confirm-delete-account?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center">
      <h1 className="mb-6 text-2xl font-bold tracking-tight">Hesap Silme Onayı</h1>
      ...
      {status === "ready" && (
        <div className="w-full rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          <p className="mb-6 text-slate-300">
            <strong className="text-white">{email}</strong> hesabını ve takip
            ettiğiniz tüm domainleri kalıcı olarak silmek istediğinizi
      ...
      {status === "confirming" && <p className="text-slate-400">Siliniyor...</p>}
