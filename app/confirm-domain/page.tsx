...
function ConfirmDomainContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/domains/confirm?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await fetch("/api/domains/confirm", {
        method: "POST",
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 text-center">
      ...
      {status === "ready" && (
        <div className="w-full rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          <p className="mb-6 text-slate-300">
            ...
            SSL Expire Tracker'a eklemek istediğinizi onaylıyor musunuz?
          </p>
          <button
      ...
      {status === "confirming" && (
        <p className="text-slate-400">Onaylanıyor...</p>
      )}
      ...
      {status === "done" && (
        <div className="w-full rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6">
          <p className="mb-4 font-medium text-emerald-400">
            Domain başarıyla eklendi ve takibe alındı!
          </p>
